...
def upgrade() -> None:
    ...
    if "report_saved_views" not in tables:
        op.create_table(
            "report_saved_views",
        ...
        existing_indexes = {idx["name"] for idx in insp.get_indexes("report_saved_views")}
        if "ix_report_saved_views_tenant_id" not in existing_indexes:
            op.create_index("ix_report_saved_views_tenant_id", "report_saved_views", ["tenant_id"])
        if "ix_report_saved_views_user_id" not in existing_indexes:
            op.create_index("ix_report_saved_views_user_id", "report_saved_views", ["user_id"])
        if "ix_report_saved_views_report_key" not in existing_indexes:
            op.create_index("ix_report_saved_views_report_key", "report_saved_views", ["report_key"])
    ...
    if "report_schedules" not in tables:
        ...
        existing_indexes = {idx["name"] for idx in insp.get_indexes("report_schedules")}
        if "ix_report_schedules_tenant_id" not in existing_indexes:
            op.create_index("ix_report_schedules_tenant_id", "report_schedules", ["tenant_id"])
        if "ix_report_schedules_user_id" not in existing_indexes:
            op.create_index("ix_report_schedules_user_id", "report_schedules", ["user_id"])
        if "ix_report_schedules_report_key" not in existing_indexes:
            op.create_index("ix_report_schedules_report_key", "report_schedules", ["report_key"])
...
def downgrade() -> None:
    ...
    if "report_saved_views" in tables:
