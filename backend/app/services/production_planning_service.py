...
def list_production_orders_enriched(
    ...
    # Bulk fetch work orders
    order_ids = [o.id for o in orders]
    all_wos = db.scalars(
        select(WorkOrder).where(
            WorkOrder.production_order_id.in_(order_ids),
            WorkOrder.tenant_id == tenant_id,
        ).order_by(WorkOrder.id.desc())
    ).all()

    wos_by_order: dict[int, list[WorkOrder]] = {}
    for wo in all_wos:
        wos_by_order.setdefault(wo.production_order_id, []).append(wo)

    # Bulk fetch machines
    machine_ids = {o.machine_id for o in orders if getattr(o, "machine_id", None)}
    for wo in all_wos:
        if wo.machine_id:
            machine_ids.add(wo.machine_id)
    machine_ids.discard(None)

    machines_map = {}
    if machine_ids:
        machines = db.scalars(
            select(Machine).where(Machine.id.in_(machine_ids), Machine.tenant_id == tenant_id)
        ).all()
    ...
    for order in orders:
        ...
        work_orders = wos_by_order.get(order.id, [])
        active_wo = work_orders[0] if work_orders else None
        m_id = active_wo.machine_id if (active_wo and active_wo.machine_id) else getattr(order, "machine_id", None)
        machine = machines_map.get(m_id) if m_id else None
        ...
        balance = max(planned - produced, 0.0)

        results.append(
            ProductionOrderListRead(
                id=order.id,
                tenant_id=order.tenant_id,
                product_id=order.product_id or 1,
                order_number=order.order_number,
                ...
                work_order_number=active_wo.work_order_number if active_wo else None,
                machine_name=machine.name if machine else None,
                machine_code=machine.code if machine else None,
