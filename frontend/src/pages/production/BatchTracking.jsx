...
import { AlertTriangle, CheckCircle2, Clock, Pause, Search, XCircle } from "lucide-react";
...
export default function BatchTracking() {
  ...
  const columns = [
    ...
    },
    {
      ...
      label: "Action",
      render: (r) => (
        <button type="button" onClick={() => openDetail(r)} className="text-xs font-semibold text-[var(--color-action-blue)] hover:underline">
          View
        </button>
      ),
    },
  ...
  return (
    <ListPageShell className="print:p-0">
      <PageHeader
        subtitle="Full batch traceability from raw material to customer dispatch."
        action={
      ...
      <div className="grid grid-cols-2 gap-3 lg:grid-cols-3 xl:grid-cols-6 print:hidden">
        ...
        <SummaryCard label="Expired" value={summary.expired} icon={AlertTriangle} color="bg-[var(--color-text-muted)]" />
      </div>
      ...
      <div className="flex flex-wrap gap-2 rounded-xl border border-[var(--color-border-soft)] bg-[var(--color-surface-muted)] px-4 py-3 print:hidden">
        {BATCH_TRACE_STEPS.map((step, i) => (
          <span key={step} className="flex items-center gap-2 text-xs text-[var(--color-text-secondary)]">
            <span className="font-semibold text-amber-600 dark:text-amber-400">{step}</span>
            {i < BATCH_TRACE_STEPS.length - 1 && <span className="text-[var(--color-text-faint)]">↓</span>}
          </span>
        ))}
      </div>
