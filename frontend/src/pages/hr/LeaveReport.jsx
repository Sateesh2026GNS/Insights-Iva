...
function GenerateReportModal({ open, onClose, onGenerate }) {
  ...
  const modal = (
    <div className="hr-leave-report__overlay" onClick={onClose} role="dialog" aria-modal="true" aria-label="Generate Leave Report">
      <div className="hr-leave-report__modal" onClick={(e) => e.stopPropagation()}>
        <div className="hr-leave-report__modal-header">
          <h2>Generate Leave Report</h2>
          <button type="button" onClick={onClose} aria-label="Close"><X className="h-5 w-5" /></button>
        </div>

        <div className="hr-leave-report__modal-body">
          <div className="hr-leave-report__form-grid">
            <div className="hr-leave-report__field">
              <label>From Date <span>*</span></label>
              <div className="hr-leave-report__date-wrap">
        ...
        <div className="hr-leave-report__modal-footer">
          <button type="button" className="hr-leave-report__cancel-btn" onClick={onClose}>Cancel</button>
          <button
            type="button"
            className="hr-leave-report__generate-btn"
...
export default function LeaveReport() {
  ...
  const load = useCallback(async (isRefresh = false) => {
    ...
    try {
      const res = await getLeaveReports({
  ...
  const handleGenerate = async (payload) => {
    const fileName = `Leave_Report_${formatDisplayDate(payload.from_date)}_to_${formatDisplayDate(payload.to_date)}.xlsx`;
    const row = {
      id: `leave-report-${Date.now()}`,
      file_name: fileName,
    ...
    try {
      await generateLeaveReport(payload);
      addToast("Leave report generated", "success");
      setRecords((prev) => [row, ...prev]);
    } catch {
      addToast("Failed to generate leave report", "error");
    }
  };
  ...
  return (
    <>
      <ListPageShell>
        <div className="hr-leave-report min-w-0">
          <div className="hr-leave-report__header">
            <h1 className="hr-leave-report__title">Leave Report</h1>
            <div className="hr-leave-report__period">
              <button type="button" className="hr-leave-report__nav-btn" onClick={() => shiftMonth(-1)} aria-label="Previous month">
                <ChevronLeft className="h-5 w-5" />
              </button>
              <span>{periodLabel}</span>
              <button type="button" className="hr-leave-report__nav-btn" onClick={() => shiftMonth(1)} aria-label="Next month">
                <ChevronRight className="h-5 w-5" />
              </button>
            </div>
            <button type="button" className="hr-leave-report__create-btn" onClick={() => setModalOpen(true)}>
              <Plus className="h-4 w-4" strokeWidth={2.5} />
              Generate Report
            </button>
          </div>

          <div className="hr-leave-report__table-wrap">
            <table className="hr-leave-report__table">
              <thead>
                <tr>
                  <th>File Name</th>
                  <th>Duration</th>
              ...
              <tbody>
                {pagedRows.length === 0 ? (
                  <tr>
                    <td colSpan={7} className="hr-leave-report__empty">No records found</td>
                  </tr>
                ) : (
                  pagedRows.map((row) => (
                    <tr key={row.id}>
                      ...
                      <td>
                        <button type="button" className="hr-leave-report__download-btn" onClick={() => handleDownload(row)} aria-label="Download">
            ...
            <div className="hr-leave-report__footer">
              <div className="hr-leave-report__page-size">
                Show
              ...
              <span className="hr-leave-report__page-info">
                Showing {showingFrom} to {showingTo} of {filteredRows.length} entries
              </span>
              <div className="hr-leave-report__pager">
