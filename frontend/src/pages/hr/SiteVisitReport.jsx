...
import { generateSiteVisitReport, getSiteVisitReports } from "../../api/hrApi";
import { exportToExcel } from "../../utils/exportUtils";
import "./siteVisitReport.css";
...
const VISIT_TYPE_OPTIONS = [
  { value: "all", label: "All Visit Type" },
...
function GenerateReportModal({ open, onClose, onGenerate }) {
  ...
  const modal = (
    <div className="hr-site-visit-report__overlay" onClick={onClose} role="dialog" aria-modal="true" aria-label="Generate Site Visit Report">
      <div className="hr-site-visit-report__modal" onClick={(e) => e.stopPropagation()}>
        <div className="hr-site-visit-report__modal-header">
          <h2>Generate Site Visit Report</h2>
          <button type="button" onClick={onClose} aria-label="Close"><X className="h-5 w-5" /></button>
        </div>

        <div className="hr-site-visit-report__modal-body">
          <div className="hr-site-visit-report__form-grid">
            <div className="hr-site-visit-report__field">
              <label>From Date <span>*</span></label>
              <div className="hr-site-visit-report__date-wrap">
        ...
        <div className="hr-site-visit-report__modal-footer">
          <button type="button" className="hr-site-visit-report__cancel-btn" onClick={onClose}>Cancel</button>
          <button
            type="button"
            className="hr-site-visit-report__generate-btn"
...
export default function SiteVisitReport() {
  ...
  const load = useCallback(async (isRefresh = false) => {
    ...
    try {
      const res = await getSiteVisitReports({
  ...
  const handleGenerate = async (payload) => {
    const fileName = `Site_Visit_Report_${formatDisplayDate(payload.from_date)}_to_${formatDisplayDate(payload.to_date)}.xlsx`;
    const row = {
      id: `site-visit-report-${Date.now()}`,
      file_name: fileName,
      ...
      visit_type: optionLabel(VISIT_TYPE_OPTIONS, payload.visit_type),
    ...
    try {
      await generateSiteVisitReport(payload);
      addToast("Site visit report generated", "success");
      setRecords((prev) => [row, ...prev]);
    } catch {
      addToast("Failed to generate site visit report", "error");
    }
  };
  ...
  const handleDownload = async (row) => {
    try {
      ...
      await exportToExcel(exportData, cols, row.file_name?.replace(/\.[^/.]+$/, "") || "Site_Visit_Report");
  ...
  if (loading) return <Loader label="Loading site visit reports..." />;

  return (
    <>
      <ListPageShell>
        <div className="hr-site-visit-report min-w-0">
          <div className="hr-site-visit-report__header">
            <h1 className="hr-site-visit-report__title">Site Visit Report</h1>
            <div className="hr-site-visit-report__period">
              <button type="button" className="hr-site-visit-report__nav-btn" onClick={() => shiftMonth(-1)} aria-label="Previous month">
                <ChevronLeft className="h-5 w-5" />
              </button>
              <span>{periodLabel}</span>
              <button type="button" className="hr-site-visit-report__nav-btn" onClick={() => shiftMonth(1)} aria-label="Next month">
                <ChevronRight className="h-5 w-5" />
              </button>
            </div>
            <button type="button" className="hr-site-visit-report__create-btn" onClick={() => setModalOpen(true)}>
              <Plus className="h-4 w-4" strokeWidth={2.5} />
              Generate Report
            </button>
          </div>

          <div className="hr-site-visit-report__table-wrap">
            <table className="hr-site-visit-report__table">
              <thead>
                <tr>
                  <th>File Name</th>
                  <th>Duration</th>
              ...
              <tbody>
                {pagedRows.length === 0 ? (
                  <tr>
                    <td colSpan={8} className="hr-site-visit-report__empty">No records found</td>
                  </tr>
                ) : (
                  pagedRows.map((row) => (
                    <tr key={row.id}>
                      ...
                      <td>
                        <button type="button" className="hr-site-visit-report__download-btn" onClick={() => handleDownload(row)} aria-label="Download">
            ...
            <div className="hr-site-visit-report__footer">
              <div className="hr-site-visit-report__page-size">
                Show
              ...
              <span className="hr-site-visit-report__page-info">
                Showing {showingFrom} to {showingTo} of {filteredRows.length} entries
              </span>
              <div className="hr-site-visit-report__pager">
