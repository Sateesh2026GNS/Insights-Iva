...
import { generateExpenseReport, getExpenseReports } from "../../api/hrApi";
...
const EXPENSE_CATEGORY_OPTIONS = [
  { value: "all", label: "All Expense Category" },
...
function GenerateReportDrawer({ open, onClose, onGenerate }) {
  ...
  const drawer = (
    <div className="hr-expense-report__overlay" onClick={onClose} role="dialog" aria-modal="true" aria-label="Generate Expense Report">
      <div className="hr-expense-report__drawer" onClick={(e) => e.stopPropagation()}>
        <div className="hr-expense-report__drawer-header">
          <h2>Generate Expense Report</h2>
          <button type="button" onClick={onClose} aria-label="Close"><X className="h-5 w-5" /></button>
        </div>

        <div className="hr-expense-report__drawer-body">
          <div className="hr-expense-report__form-grid">
            <div className="hr-expense-report__field">
              <label>From Date <span>*</span></label>
              <div className="hr-expense-report__date-wrap">
        ...
        <div className="hr-expense-report__drawer-footer">
          <button type="button" className="hr-expense-report__cancel-btn" onClick={onClose}>Cancel</button>
          <button
            type="button"
            className="hr-expense-report__generate-btn"
...
export default function ExpenseReport() {
  ...
  const handleGenerate = async (payload) => {
    const fileName = `Expense_Report_${formatDisplayDate(payload.from_date)}_to_${formatDisplayDate(payload.to_date)}.xlsx`;
    const row = {
      id: `expense-report-${Date.now()}`,
      file_name: fileName,
      ...
      expense_category: optionLabel(EXPENSE_CATEGORY_OPTIONS, payload.expense_category),
    ...
    try {
      await generateExpenseReport(payload);
      addToast("Expense report generated", "success");
      setRecords((prev) => [row, ...prev]);
    } catch {
      addToast("Failed to generate expense report", "error");
    }
  };
  ...
  const handleDownload = async (row) => {
    try {
      ...
      await exportToExcel(exportData, cols, row.file_name?.replace(/\.[^/.]+$/, "") || "Expense_Report");
  ...
  if (loading) return <Loader label="Loading expense reports..." />;

  return (
    <>
      <ListPageShell>
        <div className="hr-expense-report min-w-0">
          <div className="hr-expense-report__header">
            <h1 className="hr-expense-report__title">Expense Report</h1>
            <div className="hr-expense-report__period">
              <button type="button" className="hr-expense-report__nav-btn" onClick={() => shiftMonth(-1)} aria-label="Previous month">
                <ChevronLeft className="h-5 w-5" />
              </button>
              <span>{periodLabel}</span>
              <button type="button" className="hr-expense-report__nav-btn" onClick={() => shiftMonth(1)} aria-label="Next month">
                <ChevronRight className="h-5 w-5" />
              </button>
            </div>
            <button type="button" className="hr-expense-report__create-btn" onClick={() => setDrawerOpen(true)}>
              <Plus className="h-4 w-4" strokeWidth={2.5} />
              Generate Report
            </button>
          </div>

          <div className="hr-expense-report__table-wrap">
            <table className="hr-expense-report__table">
              <thead>
                <tr>
                  <th>File Name</th>
                  <th>Duration</th>
              ...
              <tbody>
                {pagedRows.length === 0 ? (
                  <tr>
                    <td colSpan={8} className="hr-expense-report__empty">No records found</td>
                  </tr>
                ) : (
                  pagedRows.map((row) => (
                    <tr key={row.id}>
                      ...
                      <td>
                        <button type="button" className="hr-expense-report__download-btn" onClick={() => handleDownload(row)} aria-label="Download">
            ...
            <div className="hr-expense-report__footer">
              <div className="hr-expense-report__page-size">
                Show
              ...
              <span className="hr-expense-report__page-info">
                Showing {showingFrom} to {showingTo} of {filteredRows.length} entries
              </span>
              <div className="hr-expense-report__pager">
